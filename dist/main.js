...
const SalesReport_1 = require("./reports/SalesReport");
const ProductAnalytics_1 = require("./reports/ProductAnalytics");
// ========================== SEED DATA =========================
console.log("================= SEED DATA ==================\n");
const categories = (0, SeedData_1.seedCategories)();
const products = (0, SeedData_1.seedProducts)();
const transactions = (0, SeedData_1.seedTransactions)();
console.log(`Loaded: ${categories.length} categories, ${products.length} products, ${transactions.length} transactions`);
// ========================== SALES REPORT =========================
console.log("\n================== SALES REPORT ===================\n");
const salesReport = new SalesReport_1.SalesReport(transactions);
// Total Revenue
console.log(`Total Revenue: ${salesReport.totalRevenue().toLocaleString("id-ID")}`);
...
// Revenue By Category
console.log("\nRevenue by Category:");
const revenueByCategory = salesReport.getRevenueByCategory(categories, products);
...
for (const [i, product] of topProducts.entries()) {
    console.log(`   ${i + 1}. ${product.productName.padEnd(20)} ` +
...
for (const [hour, count] of hourDist) {
    ...
    console.log(`   Jam: ${String(hour).padStart(2)}:00 | ${count} transaksi | ${bar}`);
}
// Perbandingan antara hari dengan hari lainnya
console.log("\nPerbandingan Harian:");
const comparison = salesReport.compareDaily("2026-02-01", "2026-02-03");
console.log(`   Tanggal ${comparison.date1.date} | Total: ${comparison.date1.count} transaksi | Revenue: Rp ${comparison.date1.revenue.toLocaleString("id-ID")}`);
console.log(`   Tanggal ${comparison.date2.date} | Total: ${comparison.date2.count} transaksi | Revenue: Rp ${comparison.date2.revenue.toLocaleString("id-ID")}`);
console.log(`   Selisih revenue: Rp ${comparison.revenueDiff.toLocaleString("id-ID")}`);
...
// =================== PRODUCT ANALYTICS ===================
console.log("\n================ PRODUCT ANALYTICS =================\n");
const analytics = new ProductAnalytics_1.ProductAnalytics(products);
// Summary
